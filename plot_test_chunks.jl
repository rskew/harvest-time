...
# Yeild Per Year
yields_per_year = Array{Float64}(years_plus_chunks)
prop_yields = zeros(length(years_plus_chunks),length(properties))
age_tally = zeros(length(yields[1,:]))
year_profit = zeros(years_plus_chunks)
...
#YIELD PER YEAR
display(Plots.bar(years_plus_chunks,yields_per_year,
